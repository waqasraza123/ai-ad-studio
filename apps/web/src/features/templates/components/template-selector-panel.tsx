...
export async function TemplateSelectorPanel({
  currentTemplateId,
  projectId,
  templates
  ...
  return (
    <SurfaceCard className="p-6">
      <p className="text-sm uppercase tracking-[0.24em] text-slate-400">
        {t("templates.panel.eyebrow")}
      </p>
      <h2 className="mt-3 text-2xl font-semibold tracking-[-0.03em] text-white">
        {t("templates.panel.title")}
      </h2>
      <p className="mt-3 text-sm leading-7 text-slate-400">
        {t("templates.panel.description")}
      </p>

      <form action={action} className="mt-6 space-y-5">
        <label className="grid gap-2">
          <span className="text-sm text-slate-300">{t("templates.panel.label")}</span>
          <select
        ...
        <FormSubmitButton pendingLabel={t("templates.panel.pending")}>
          {t("templates.panel.save")}
        </FormSubmitButton>
      </form>
    </SurfaceCard>
  )
